...
# Azure Document Intelligence (optional — lazy-imported if configured)
_AZURE_DOC_INTEL_EXTRACTOR = None

def _get_azure_doc_intel_extractor():
    """Return an AzureDocIntelligenceExtractor if credentials are set, else None."""
    global _AZURE_DOC_INTEL_EXTRACTOR
    if _AZURE_DOC_INTEL_EXTRACTOR is not None:
        return _AZURE_DOC_INTEL_EXTRACTOR
    ...
    try:
        ...
        _AZURE_DOC_INTEL_EXTRACTOR = AzureDocIntelligenceExtractor(
            endpoint=endpoint,
            key=key,
            model_id=os.getenv("DOC_INTEL_MODEL_ID", "prebuilt-layout")
        )
        return _AZURE_DOC_INTEL_EXTRACTOR
    except Exception as exc:
        logger.warning("Azure Document Intelligence not available: %s", exc)
        _AZURE_DOC_INTEL_EXTRACTOR = None
        return None
...
class DocumentIngestionPipeline:
    ...
    def __init__(self):
        ...
        self._azure_extractor = _get_azure_doc_intel_extractor()
    ...
    def ingest_documents(self, input_path: Path) -> list[dict]:
        ...
        # Attempt Azure Document Intelligence extraction first if configured
        if self._azure_extractor is not None:
            logger.info('Using Azure Document Intelligence for extraction')
            azure_results = []
            for f in files:
                try:
                    result = self._azure_extractor.extract(str(f))
                    azure_results.append(result)
                except Exception as exc:
                    logger.warning('Azure FR failed for %s: %s', f, exc)
                    # fall through to built-in processor for this file
            if azure_results and len(azure_results) == len(files):
                # All files succeeded with FR; return results directly
                logger.info('All documents extracted via Azure FR')
                return azure_results
            # If some failed or mixed, fall back to built-in processor below
        
        # Built-in processor fallback
